...
def get_yahoo_symbol(kod, pazar):
    ...
    if "BIST" in str(pazar):
        return f"{kod}.IS" if not str(kod).endswith(".IS") else kod
    elif "KRIPTO" in str(pazar):
        return f"{kod}-USD" if not str(kod).endswith("-USD") else kod
...
# --- ZIRHLI SAYI ÇEVİRİCİ (YENİ VERSİYON) ---
def smart_parse(text_val):
    """
    Her türlü sayı formatını düzeltir.
    Özellikle Google Sheets'in '30,26' değerini 3026 gibi göndermesi durumunda
    otomatik olarak 3026 -> 30.26'ya çeviren ekstra bir hack içerir.
    """
    if text_val is None:
        return 0.0
    ...
    # 1) Hem nokta hem virgül varsa (genelde 1.234,56 gibi Avrupa formatı)
    if "," in s and "." in s:
        # Binlik ayracını (.) kaldır, virgülü (,) ondalık olarak . yap
        s = s.replace(".", "").replace(",", ".")

    # 2) Sadece virgül varsa (30,26 gibi)
    elif "," in s and "." not in s:
        s = s.replace(",", ".")

    # 3) Sadece nokta varsa -> zaten 30.26 gibi, olduğu gibi kalsın
    ...
    # --- ÖZEL HACK: 3026 GİBİ 100x KAYMIŞ DEĞERLERİ DÜZELT ---
    # Eğer orijinal stringte virgül vardı ama sonuç büyük ve tam sayı çıktıysa
    # (ör: '30,26' bir şekilde '3026' olmuşsa), 100'e böl (3026 -> 30.26)
    if ("," in raw) and (val >= 1000) and (val == int(val)):
        val = val / 100.0
...
def get_tickers_data(df_portfolio, usd_try):
    ...
    try:
        ...
        for name, symb in market_symbols:
            val = get_val(symb, name)
            if val:
                market_html += f"{val} &nbsp;|&nbsp; "
        ...
        if total_cap > 0:
            ...
            market_html += f'BTC.D: <span style="#f2a900">% {btc_d:.2f}</span> &nbsp;|&nbsp; '
        ...
        if portfolio_symbols:
            for name, symb in portfolio_symbols.items():
                val = get_val(symb, name)
                if val:
...
ANALYSIS_COLS = [
    "Kod", "Pazar", "Tip", "Adet", "Maliyet", "Fiyat",
    "PB", "Değer", "Top. Kâr/Zarar", "Top. %", "Gün. Kâr/Zarar", "Notlar"
]
...
# --- HESAPLAMA MOTORU (BIST 100x FIX’Lİ) ---
def run_analysis(df, usd_try_rate, view_currency):
    ...
    for _, row in df.iterrows():
        ...
        # 🔥 BIST için 3026 gibi sapık maliyetleri düzelt
        if "BIST" in str(pazar):
