...
# --- ZIRHLI SAYI ÇEVİRİCİ (TEMİZ & GÜVENLİ) ---
def smart_parse(text_val):
    """
    Çeşitli sayı formatlarını (30,26 / 30.26 / 1.234,56 vb.) float'a çevirir.
    Google Sheets'ten gelen ham stringleri işler. Saçma karakterleri temizler.
    """
    if text_val is None:
        return 0.0
    ...
    # Hem nokta hem virgül varsa -> Avrupa formatı varsay: 1.234,56
    if "," in s and "." in s:
        s = s.replace(".", "").replace(",", ".")
    # Sadece virgül varsa -> 30,26 gibi
...
def get_data_from_sheet():
    """
    ÖNEMLİ: get_all_values kullanıyoruz -> Hücrede ne yazıyorsa STRING olarak geliyor.
    Yani 30,26 gerçekten '30,26' olarak bize ulaşıyor, 3026'e çevrilmiyor.
    """
    try:
        scope = [
        ...
        values = sheet.get_all_values()  # <-- kritik değişiklik
        if not values:
...
def add_sale_record(date, code, market, qty, price, cost, profit):
    try:
        ...
        client = gspread.authorize(creds)
        sheet = client.open(SHEET_NAME).worksheet("Satislar")
        row = [str(date), code, market, str(qty), str(price), str(cost), str(profit)]
...
def save_data_to_sheet(df):
    """
    DF'i sheet'e yazarken değerleri aynen gönderiyoruz.
    Okurken get_all_values kullandığımız için zaten hepsini string olarak geri alacağız.
    """
    scope = [
        "https://spreadsheets.google.com/feeds",
        "https://www.googleapis.com/auth/drive",
